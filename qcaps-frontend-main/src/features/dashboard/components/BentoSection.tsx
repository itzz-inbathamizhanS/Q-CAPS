...
export const BentoSection: React.FC<BentoSectionProps> = ({ liveRecommendation }) => {
  ...
  const displayTitle = liveRecommendation?.title || recommendedLearning.title;
  const displayDesc = liveRecommendation?.reason || recommendedLearning.description;
  const displayBadge = liveRecommendation?.priority
    ? `Priority: ${liveRecommendation.priority}`
    : recommendedLearning.badgeText;
