...
def run_api_tests():
    ...
    def register_and_login(name):
        res_reg = client.post("/api/auth/register", json={"name": name, "password": "password"})
        check(f"Create {name} status code == 200", res_reg.status_code == 200)
        user_id = res_reg.json()["id"]
        
        res_login = client.post("/api/auth/login", json={"name": name, "password": "password"})
        token = res_login.json()["access_token"]
        headers = {"Authorization": f"Bearer {token}"}
        return user_id, headers

    u1_id, h1 = register_and_login("Alice")
    u2_id, h2 = register_and_login("Bob")
    u3_id, h3 = register_and_login("Charlie")
    u4_id, h4 = register_and_login("Dave")
    ...
    # Alice: pqc is 30% (lowest score) -> track_b_b9_pqc_fundamentals (Critical)
    res_alice_rec = client.get(f"/api/users/{u1_id}/recommendation", headers=h1)
    alice_rec = res_alice_rec.json()
    check("Alice recommended course == track_b_b9_pqc_fundamentals", alice_rec.get("course_id") == "track_b_b9_pqc_fundamentals")
    check("Alice recommendation priority == Critical", alice_rec.get("priority") == "Critical")

    # Bob: QF (100%), PQC (90%), PS (80%) -> No major skill gap
    res_bob_rec = client.get(f"/api/users/{u2_id}/recommendation", headers=h2)
    ...
    # Charlie: Unattempted + Critical RSA → Critical recommendation for PQC (track_b_b9_pqc_fundamentals)
    res_charlie_rec = client.get(f"/api/users/{u3_id}/recommendation", headers=h3)
    charlie_rec = res_charlie_rec.json()
    check("Charlie recommended course == track_b_b9_pqc_fundamentals", charlie_rec.get("course_id") == "track_b_b9_pqc_fundamentals")
    ...
    res_dave_rec = client.get(f"/api/users/{u4_id}/recommendation", headers=h4)
    dave_rec = res_dave_rec.json()
    check("Dave recommended course == track_b_b9_pqc_fundamentals", dave_rec.get("course_id") == "track_b_b9_pqc_fundamentals")
    ...
    res_alice_profile = client.get(f"/api/users/{u1_id}/profile", headers=h1)
    alice_profile = res_alice_profile.json()
    check("Profile ID is correct format", type(alice_profile.get("id")) is int)
    check("Profile name matches", alice_profile.get("name") == "Alice")
    check("Profile readiness score is computed", type(alice_profile.get("readiness_score")) is int)
