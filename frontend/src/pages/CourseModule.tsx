...
const MarkdownRenderer: React.FC<{ content: string; className?: string }> = ({ content, className }) => {
  return (
    <div className={className} style={{ fontSize: '15px', lineHeight: 1.7, color: 'var(--color-text-primary, #171821)' }}>
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        components={{
          p: ({ children }) => <p style={{ marginBottom: '12px', lineHeight: 1.7 }}>{children}</p>,
          strong: ({ children }) => <strong style={{ fontWeight: 700, color: 'var(--color-text-primary, #0f172a)' }}>{children}</strong>,
          ...
          ul: ({ children }) => (
            <ul style={{ paddingLeft: '22px', marginBottom: '12px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {children}
            </ul>
          ),
          ol: ({ children }) => (
            <ol style={{ paddingLeft: '22px', marginBottom: '12px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {children}
            </ol>
          ),
          li: ({ children }) => <li style={{ lineHeight: 1.6, color: 'var(--color-text-primary, #1e293b)' }}>{children}</li>,
          blockquote: ({ children }) => (
            <blockquote
              style={{
                borderLeft: '3px solid var(--color-primary, #5427e6)',
                paddingLeft: '14px',
                margin: '14px 0',
          ...
          hr: () => <hr style={{ margin: '18px 0', border: 'none', borderTop: '1px solid var(--color-border, #e2e8f0)' }} />,
        }}
      >
        {content}
      </ReactMarkdown>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '28px', marginBottom: '40px' }}>
              ...
              {sec.interactiveCallout && (
                ...
                >
                  ...
                  {expandedCallout[sec.id] && (
                    ...
                    >
                      <MarkdownRenderer content={sec.interactiveCallout} />
                    </div>
                  )}
                </div>
              )}
        ...
        {currentMod.wrapUp?.summary && (
          ...
          >
            ...
            <MarkdownRenderer content={currentMod.wrapUp.summary} />
          </div>
        )}
      </div>
          ...
          {hasQuiz ? (
            <Button
              ...
              style={{
                ...
                alignItems: 'center',
                gap: '8px',
                backgroundColor: isPassed ? 'var(--color-emerald)' : 'var(--color-primary)'
              }}
            >
